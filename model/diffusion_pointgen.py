...
class UNetBlock(nn.Module):
    def __init__(self, in_channels, out_channels, time_emb_dim, num_points):
        ...
        self.relu = nn.ReLU()
        # Additional layers to transform 't_embed' to match 'x' dimensions
        self.time_emb_transform = nn.Linear(time_emb_dim, out_channels // 2)  # Adjust for reshaping
        # Adjust cond embedding size to match 'x' after convolutions
        self.embedding_adjust = nn.Linear(512, 256)

    def forward(self, cond, x, t_embed):
        # Reshape and permute 'x' as per the requirement
        print(f"Shape of x before self.conv1: {x.shape}")
        x = x.view(1, 16, 1)  # Adjust shape as specified
        x = x.permute(0, 2, 1)  # Permute to match Conv1d input expectations
        ...
        x = self.conv2(x)
        x = self.norm2(x)
        x = self.relu(x)  # Shape now is [batch_size, out_channels, seq_len]

        # Adjust 't_embed' to have the correct shape to add to 'x'
        t_embed = self.time_emb_transform(t_embed)  # Transform 't_embed'
        print("Embedding shape!")
        print(t_embed.shape)
        t_embed = t_embed.view(1, self.out_channels, 16)  # Reshape to match 'x' dimensions after convolutions
        ...
        print("COND")
        print(cond.shape)
        cond = self.embedding_adjust(cond)  # Adjust embedding size

        # Ensure correct shape for MultiheadAttention
        x = x.permute(0, 1, 2)  # Shape [seq_len, batch_size, embedding_size]
        cond = cond.permute(1, 0, 2)  # Also adjust 'cond'
...
class DiffusionModel(nn.Module):
    def __init__(self, hidden_dim, output_dim, num_points, num_timesteps):
        ...
        self.cond_embedding = nn.Sequential(
            nn.Linear(2048 * 3 * 3, 1),  # Adjusting to reflect the flattened, pooled output size
            nn.ReLU()
        )
        ...
        self.down1 = UNetBlock(hidden_dim, hidden_dim * 2, hidden_dim, self.num_points)
        self.down2 = UNetBlock(hidden_dim * 2, hidden_dim * 4, hidden_dim, self.num_points)
        self.down3 = UNetBlock(hidden_dim * 4, hidden_dim * 8, hidden_dim, self.num_points)
        self.bottleneck = UNetBlock(hidden_dim * 8, hidden_dim * 16, hidden_dim, self.num_points)
        self.up1 = UNetBlock(hidden_dim * 16, hidden_dim * 8, hidden_dim, self.num_points)
        self.up2 = UNetBlock(hidden_dim * 8, hidden_dim * 4, hidden_dim, self.num_points)
        self.up3 = UNetBlock(hidden_dim * 4, hidden_dim * 2, hidden_dim, self.num_points)
...
print(f"Model save directory '{model_save_dir}' prepared.")

# Initialize Weights & Biases
wandb.init(project="diffusom")
print("WandB initialized.")

# Dataset and DataLoader setup
print("Initializing dataset and dataloader...")
dataset = SceneCompletionDataset(0, num_points)  # Assuming seq is set to 0 for simplicity
dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
print(f"Dataset and DataLoader ready. Dataset size: {len(dataset)}")
...
if use_ensemble:
    ...
    for i in range(num_ensemble):
        print(f"Initializing model {i+1}/{num_ensemble}...")
        model = DiffusionModel(hidden_dim, output_dim, num_points, num_timesteps).to(device)
        optimizer = optim.Adam(model.parameters(), lr=learning_rate)
        models.append(model)
        optimizers.append(optimizer)
    print(f"Initialized {num_ensemble} models for the ensemble.")
else:
    ...
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
    print("Single model initialized.")

criterion = nn.MSELoss()
print("Loss function (MSELoss) set.")
...
if use_ensemble:
    for i in range(num_ensemble):
        ...
        # Assuming the `train` function includes necessary print statements for training progress
